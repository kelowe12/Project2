...
Level.prototype.obstacleAt = function(pos, size) {
  ...
  for (var y = yStart; y < yEnd; y++) {
    for (var x = xStart; x < xEnd; x++) {
      ...
      if (fieldType && fieldType != "floater" && fieldType != "fire" && fieldType != "ground")
      return fieldType;
    }
  }
};
...
var wobbSpeed = 2, wobbDist = 3.5;

sCoin.prototype.act = function(step) {
...
Normal.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if(obstacle == "portal"){
    this.pos = new Vector(41, 5);
 }
 if(obstacle == "portal2"){
   this.pos = new Vector(20, 37);
 }
 if(obstacle == "portal3"){
   this.pos = new Vector(15, 20);
}
 if(obstacle == "portal4"){
   this.pos = new Vector(98, 30);
}
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "poison" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin" || type == "scoin") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any coins left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "coin" || actor.type == "scoin";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
